...
def checkNextMoves(startingpoint):
...
checkNextMoves(flatCavern.index("S"))
...
ACost = []
for pos, x in enumerate(flatCavern):
    print("Position: ", pos)
    if x == "a":
        routeCosts = [sys.maxsize] * len(flatCavern)
        checkNextMoves(pos)
        ACost.append(routeCosts[flatCavern.index("E")])
print("Min A cost: ", min(ACost))
